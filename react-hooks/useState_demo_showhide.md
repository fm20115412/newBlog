...
function App({ posts }) {
  let [show, setShow] = React.useState(0);
  return (
    <div>
      <ul>
        {posts.map(post => {
          let { id, img, text } = post;
          let isOpen = show === id;
          return (
            <li key={id} className={classNames({ open: isOpen })}>
              <img src={img} alt="img" />
              <p>{isOpen ? text : text.substring(0, 100) + "..."}</p>
              {isOpen ? null : (
                <button onClick={() => setShow(id)}>open</button>
              )}
            </li>
          );
        })}
